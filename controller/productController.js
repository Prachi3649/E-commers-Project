...
//         // Construct a unique key based on request parameters
//         const redisKey = JSON.stringify(req.body);
//         console.log("redisKey", redisKey);

//         // Check if the data is already cached in Redis
//         const cachedData = await GET_ASYNC(redisKey);

//         if (cachedData) {
//             console.log("Data retrieved from Redis cache");
//             return res.status(200).send({ status: true, message: ' Get Successfully', data: JSON.parse(cachedData) });
//         }

//         if (product_Name) {
//             data["product_Name"] = { $regex: product_Name.trim() };
//         }

//         if (description) {
//             data["description"] = { $regex: description.trim() };
//         }

//         if (category) {
//             data["category"] = { $regex: category.trim() };
//         }

//         if (price) {
//             data["price"] = { $regex: price.trim() };
//         }

//         const get_product = await product_Model.find(data)
...
//     } else {
//         return res.status(500).send({ status: false, message: 'Redis client is not ready ===>' });
//     }
//     } catch (err) {
//         console.log("error", err);
//         res.status(500).json({ error: "Internal Server Error" });
